...
const Resume = forwardRef((props, ref) => {
  ...
  const skillContent=(value)=>{
    if (value) {
      let tempArr = value.split(',');
      return tempArr.map((item => (<span className={styles.skillItem}> {item} </span>)))
  ...
  const sectionDiv = {
    ...
    [sections.skill]: (
      ...
      >
        ...
        <div className={styles.content}>
          <p className={styles.overview}>{skillContent(info.skill?.detail)} </p>
        </div>
      </div>
    ),
    [sections.other]: (
  ...
  useEffect(() => {
    setColumns([
      ...
      [sections.skill, sections.workExp, sections.achievement, sections.other ,],
    ]);
  }, []);
